...
class PathPlanningDrawer():
    ...
    def draw_drone(self, drone, cur_step):
        ...
        if config.SHOW_COMPLETE_PATH:
            self.__draw_complete_path(drone)
    ...
    def __draw_complete_path(self, drone):
        
        next_path = drone.path[1:]
        next_path.append(drone.path[0])
        
        color = stddraw.BLUE if drone.identifier == 0 else stddraw.BLACK

        for (xs, ys), (xt, yt) in zip(drone.path, next_path):
            stddraw.setPenRadius(0.0055)
            stddraw.setPenColor(c=color)
            stddraw.point(xs, ys); stddraw.point(xt, yt)
            stddraw.setPenColor()
            self.__reset_pen()
